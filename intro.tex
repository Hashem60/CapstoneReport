...
\begin{figure*}[h]
    \centering
    \begin{tikzpicture}
        % Adjust gradient background size to match QR code
        % \shade[left color=blue!30, right color=red!30] (-6,-6) rectangle (6,6); % 3cm × 3cm
        
        % Explicitly resize the QR code to 3cm × 3cm
        \node at (0,0) {\resizebox{10cm}{6cm}{\qrcode{https://github.com/Hashem60/CapstoneReport}}};
        
        % Resize logo to match (e.g., 20% of QR code size)
        \node at (0,0) {\includegraphics[width=3cm]{doc/logo.png}}; % Adjust width as needed
      \end{tikzpicture}
