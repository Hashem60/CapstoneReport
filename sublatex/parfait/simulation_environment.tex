...
\lstset{language=XML}
\subsection{Introduction to the Simulation Environment}
...
ensuring that the robot's interaction with its surroundings is as realistic as possible. Key components of the environment layout include the following:
    ...
    \paragraph{Flooring and Pathways:}
...
\paragraph{Obstacles and Load Platforms:}
...
\begin{figure}[H]
    ...
    \paragraph{Random Clutter Generation:} Procedural generation methods were used to add \emph{random clutter}, 
    simulating items or obstacles that may appear unpredictably, requiring the robot to adapt to ever-changing environments. 
    
    \paragraph{Real-World Simulation Features:} External environmental factors such as wind were not simulated,  
